...
const client = new Client('b3dbee58-65a6-4627-963f-f9dc16675636');
...
app.post('/register', async (req, res) => {
  ...
  if (res.locals.loggedIn) {
    ...
    try {
      const {
        email, password, confirmation, custom_data: customData,
      } = req.body;
...
app.get('/users', utils.checkIfAdmin, async (req, res) => {
  console.log('---------');
...
app.get('/users/:id', utils.checkIfAdmin, async (req, res) => {
  console.log('---------');
  console.log('Users API hit');
  try {
...
app.post('/users/:id/delete', utils.checkIfAdmin, async (req, res) => {
  console.log('---------');
  console.log('Users API hit');
  try {
...
app.post('/location', utils.checkIfLoggedIn, async (req, res) => {
  console.log('---------');
  console.log('Location API hit');
  try {
    req.body.posted_by = req.session.userId;
    const id = await db.createLocation(req.body);
    res.status(200).send({
      id,
    });
  } catch (error) {
...
app.post('/book', utils.checkIfLoggedIn, async (req, res) => {
  console.log('---------');
  console.log('Book API hit');
  try {
    const id = await db.createBook(req.body);
    res.status(200).send({
      id,
    });
  } catch (error) {
...
app.get('/feed', async (req, res) => {
  ...
  try {
    const items = await db.getFeed(req.query);
...
app.post('/order', utils.checkIfLoggedIn, async (req, res) => {
  console.log('---------');
  console.log('Order API hit');
  try {
    req.body.user_id = req.session.userId;
    req.body.payment_link = 'https://p-y.tm/JXw-Dn8';
    const id = await db.createOrder(req.body);
...
app.get('/items', utils.checkIfAdmin, async (req, res) => {
  console.log('---------');
  console.log('Item API hit');
  try {
...
app.get('/items/:id', utils.checkIfLoggedIn, async (req, res) => {
  console.log('---------');
  console.log('Item API hit');
  try {
    const items = await db.getItems(req.param.id);
    if (items.length) {
...
app.get('/orders/all', utils.checkIfAdmin, async (req, res) => {
  console.log('---------');
...
app.get('/orders', utils.checkIfLoggedIn, async (req, res) => {
  console.log('---------');
  console.log('Users Orders API hit');
  try {
...
app.get('/orders/:id', utils.checkIfLoggedIn, async (req, res) => {
  console.log('---------');
  console.log('Orders API hit');
  try {
    const orders = await db.getOrders(req.param.id);
    if (orders.length) {
...
app.get('/books/search', utils.checkIfLoggedIn, async (req, res) => {
  console.log('---------');
...
app.get('/forms/:id/submit', async (req, res) => {
  ...
  res.render('form-submit', { error: null, form });
});
